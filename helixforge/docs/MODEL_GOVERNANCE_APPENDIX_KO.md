...
- **Provenance.** Every model/heuristic output is source-typed; baseline models are labeled BASELINE_MODEL_OUTPUT and never described as validated.
- **Evaluation cadence.** Studies re-run on any model, prompt, or safety change (see Change Control Log).
