...
class EvidenceMinerAgent(BaseAgent):
    ...
    def run(self, ctx: AgentContext) -> AgentOutput:
        ...
        evidence: list[dict] = []
        for index, it in enumerate(res.get("items", [])[: ctx.max_results], start=1):
            ...
            source_id = it.get("pmid") or f"item-{index}"
            ev = {
                "id": f"ev-{ctx.workflow_run_id}-{source_id}",
        ...
        if ctx.inj("contradictory_evidence") and evidence:
            contra = {
                "id": f"ev-{ctx.workflow_run_id}-contradictory-demo",
                "project_id": ctx.project_id, "workflow_run_id": ctx.workflow_run_id,
                "created_at": utcnow(),
