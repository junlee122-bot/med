...
export const api = {
  ...
  // ---- Phase 7: hybrid Fable layer ----
  llmConfig: () => get<any>('/api/llm/config'),
  llmHealth: () => get<any>('/api/llm/health'),
  llmRouter: (mode?: string) => get<any>(`/api/llm/router${mode ? `?mode=${mode}` : ''}`),
  llmRouterPreview: (purpose: string, mode?: string) => post<any>('/api/llm/router/preview', { purpose, mode }),
  llmSetMode: (mode: string) => post<any>('/api/llm/router/set-mode', { mode }),
  llmCosts: (run_id?: string) => get<any>(`/api/llm/costs${run_id ? `?run_id=${run_id}` : ''}`),
  llmResetSession: () => post<any>('/api/llm/costs/reset-session', {}),
  llmLedger: (run_id?: string) => get<any>(`/api/llm/ledger${run_id ? `?run_id=${run_id}` : ''}`),
  llmPricing: () => get<any>('/api/llm/pricing'),
  llmPromptTemplates: () => get<any>('/api/llm/prompt-templates'),
  llmLiveSmoke: (body: any) => post<any>('/api/llm/live-smoke', body),
  planHybrid: (body: any) => post<any>('/api/workflow/plan-hybrid', body),
  runHybridPipeline: (body: any) => post<any>('/api/workflow/run-hybrid-agentic-pipeline', body),
  hypothesesHybrid: (run_id: string) => get<any>(`/api/hypotheses/run/${run_id}/hybrid`),
  semanticCritic: (run_id: string) => get<any>(`/api/critic/semantic/run/${run_id}`),
  rediscoveryScenarios: () => get<any>('/api/rediscovery/scenarios'),
  rediscoveryComparators: (scenario_id: string) => get<any>(`/api/rediscovery/comparators?scenario_id=${scenario_id}`),
  rediscoveryRun: (body: any) => post<any>('/api/rediscovery/run', body),
  rediscoveryReport: (id: string) => get<any>(`/api/rediscovery/run/${id}/report`),
  optimizationRun: (body: any) => post<any>('/api/optimization-loop/run', body),
  optimizationTrace: (id: string) => get<any>(`/api/optimization-loop/run/${id}/trace`),
  optimizationReport: (id: string) => get<any>(`/api/optimization-loop/run/${id}/report`),
